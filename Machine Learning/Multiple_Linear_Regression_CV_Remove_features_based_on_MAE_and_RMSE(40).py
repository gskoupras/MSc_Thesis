# =============================================================================
# Multiple_Linear_Regression_CV_Remove_features_based_on_MAE_and_RMSE.py
# =============================================================================
import pandas as pd
import numpy as np
...
while terminal == 1:
    ...
    for i in range(0, len(features)):
        ...
        lr = LinearRegression()
        scores = cross_validate(lr, X_test, y, cv=tscv.split(X_test),
                                scoring=('neg_mean_squared_error',
                                'neg_mean_absolute_error'))
...
while (terminal == 1) and (len(features) > 1):
...
#               'when removing features based on MAE and RMSE')
#axes.legend(loc='best')
#axes.grid(True)
#axes.autoscale()
## fig.savefig('How Mean Absolute Error evolves '
##             'when removing features based on RMSE and MAE',
...
axes2.annotate(str(min(rmse_evo)),
               xy=(np.argmin(rmse_evo), min(rmse_evo)+0.015), xycoords='data',
               xytext=(np.argmin(rmse_evo), min(rmse_evo)+0.09), textcoords='data',
               arrowprops=dict(arrowstyle="<-", facecolor='black'),
               horizontalalignment='center', verticalalignment='top',
               fontsize=16)
