# =============================================================================
# Random_Forest_Regression_CV_Remove_features_based_on_MAE.py
# =============================================================================
import pandas as pd
import numpy as np
import time
...
scores = cross_validate(rfr, X, y, cv=tscv.split(X),
...
while terminal == 1:
    ...
    for i in range(0, len(features)):
        ...
        rfr = RandomForestRegressor(n_estimators=200, random_state=0)
        scores = cross_validate(rfr, X_test, y, cv=tscv.split(X_test),
                                scoring=('neg_mean_squared_error',
                                'neg_mean_absolute_error'))
...
axes.annotate(str(min(mae_evo)),
              xy=(np.argmin(mae_evo), min(mae_evo)+0.08), xycoords='data',
              xytext=(np.argmin(mae_evo), min(mae_evo)+0.3), textcoords='data',
...
axes2.annotate(str(min(rmse_evo)),
               xy=(np.argmin(rmse_evo), min(rmse_evo)+0.03), xycoords='data',
               xytext=(np.argmin(rmse_evo), min(rmse_evo)+0.15), textcoords='data',
               arrowprops=dict(arrowstyle="<-", facecolor='black'),
               horizontalalignment='center', verticalalignment='top',
               fontsize=16)
