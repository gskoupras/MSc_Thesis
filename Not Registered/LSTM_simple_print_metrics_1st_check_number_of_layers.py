...
drop = 0.2  # To tackle regularization
r = 2
...
for i in range(0, len(n_layers)):
    axes.scatter(i, mae[i], marker="o", s=size, label=n_layers[i])
axes.set_xlabel('Basic Method')
axes.set_ylabel('Mean Absolute Error')
axes.set_title('Basic Methods Performance')
axes.set_xticks(list(range(0, len(n_layers))))
axes.set_xticklabels(list(range(1, len(n_layers)+1)))
...
axes = fig.add_axes([0.1, 0.1, 0.8, 0.8])
for i in range(0, len(n_layers)):
    axes.scatter(i, rmse[i], marker="o", s=size, label=n_layers[i])
axes.set_xlabel('Basic Method')
...
axes = fig.add_axes([0.1, 0.1, 0.8, 0.8])
for i in range(0, len(n_layers)):
    axes.scatter(i, rt[i], marker="o", s=size, label=n_layers[i])
axes.set_xlabel('Basic Method')
