...
for i in n_neurons:
    ...
    lstm.add(Dense(i, activation='relu'))
...
for i in range(0, len(n_neurons)):
    axes.scatter(i, mae[i], marker="o", s=size, label=n_neurons[i])
axes.set_xlabel('Basic Method')
axes.set_ylabel('Mean Absolute Error')
axes.set_title('Basic Methods Performance')
axes.set_xticks(list(range(0, len(n_neurons))))
axes.set_xticklabels(n_neurons)
...
axes = fig.add_axes([0.1, 0.1, 0.8, 0.8])
for i in range(0, len(n_neurons)):
    axes.scatter(i, rmse[i], marker="o", s=size, label=n_neurons[i])
axes.set_xlabel('Basic Method')
...
axes = fig.add_axes([0.1, 0.1, 0.8, 0.8])
for i in range(0, len(n_neurons)):
    axes.scatter(i, rt[i], marker="o", s=size, label=n_neurons[i])
axes.set_xlabel('Basic Method')
