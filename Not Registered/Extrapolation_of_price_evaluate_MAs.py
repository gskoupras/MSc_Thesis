...
def predict_with_extrapolation_1h(y, smas, degree=1):
    ...
    for i in smas:
        ...
        mae = np.append(mae, round(metrics.mean_absolute_error(y_new, y_pred), r))
        rmse = np.append(rmse, round(np.sqrt(metrics.mean_squared_error(y_new, y_pred)), r))
        mse = np.append(mse, round(metrics.mean_squared_error(y_new, y_pred), r))
        da = np.append(da, round(direction_accuracy(y_new, y_pred), r))
        rt = np.append(rt, round(time.time() - start_time, r))
    return mae, rmse, mse, da, rt


sma = list(range(10, 31))
deg = 1
...
axes.set_xticks(range(min(sma), max(sma)+1), 2)
