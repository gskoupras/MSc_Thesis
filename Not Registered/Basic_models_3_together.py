...
axes2 = axes.twinx()
for i in range(0, len(labels)):
    axes2.scatter(i, rmse[i], marker="s", s=size)
...
axes3 = axes.twinx()
for i in range(0, len(labels)):
    axes3.scatter(i, rt[i], marker="P", s=size)
rtl = axes3.scatter(0, rt[0],
                      marker="P",
                      color=plt.rcParams['axes.prop_cycle'].by_key()['color'][7],
                      s=size)
...
axes3.set_ylabel('Running Time (s)', fontsize=16)
...
leg2 = fig.legend([mael, rmsel, rtl],
                  ['Mean Absolute Error',
                   'Root Mean Squared Error', 'Running Time'],
                  bbox_to_anchor=(1.005, 1.01),
                  bbox_transform=axes.transAxes, fontsize=12)
axes.add_artist(leg1)
axes.grid(True)
axes.autoscale()
#fig.savefig('Basic Methods Performance',
