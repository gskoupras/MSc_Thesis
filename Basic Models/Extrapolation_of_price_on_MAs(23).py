# =============================================================================
# Extrapolation_of_price_on_MAs.py
# =============================================================================
import pandas as pd
import numpy as np
...
data.loc[:, 'SMA'] = data['Offers'].rolling(window=10).mean()
data.loc[:, 'SMA'].fillna(data.loc[:, 'SMA'].mean(), inplace=True)
...
def predict_with_extrapolation_1h(y, y_real, degree):
    ...
    for i in degree:
        ...
        for j in range(i+2, len(y)):
            ...
            f = np.poly1d(z)
            y_pred.iloc[j] = f(i+3)
        mae = np.append(mae, round(metrics.mean_absolute_error(y_real, y_pred), r))
        rmse = np.append(rmse, round(np.sqrt(metrics.mean_squared_error(y_real, y_pred)), r))
        mse = np.append(mse, round(metrics.mean_squared_error(y_real, y_pred), r))
        da = np.append(da, round(direction_accuracy(y_real, y_pred), r))
        rt = np.append(rt, round(time.time() - start_time, r))
    return mae, rmse, mse, da, rt


degrees = range(1, 9)
mae, rmse, mse, da, rt = predict_with_extrapolation_1h(data['SMA'],
                                                       data['Offers'],
                                                       degree=degrees)
...
axes.set_title('Polynomial Methods Performance', fontsize=18)
axes.set_xticks(range(min(degrees), max(degrees)+1))
fig.legend(bbox_to_anchor=(1, 0.3),
           bbox_transform=axes.transAxes, fontsize=16)
axes.grid(True)
axes.autoscale()
