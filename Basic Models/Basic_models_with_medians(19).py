...
def predict_with_median_of_previous_prices(y, num_prev):
    ...
    for i in num_prev:
        ...
        y_pred = pd.DataFrame.copy(y)
        for j in range(i+1, len(y)):
            y_pred.iloc[j] = y.iloc[j-i-1:j-1].median()
        y_pred.fillna(y_pred.median(), inplace=True)
...
mae, rmse, mse, da, rt = predict_with_median_of_previous_prices(data['Offers'],
                                                                n_prev)
...
axes.set_title('Median of Previous Observations Methods Performance',
               fontsize=18)
...
axes.grid(True)
axes.autoscale()
# fig.savefig('Median of Previous Observations Methods Performance',
...
axes.set_title('Median of Previous Observations Methods Performance',
               fontsize=18)
axes.set_xticks(range(min(n_prev), max(n_prev)+1), 5)
fig.legend(bbox_to_anchor=(0.46, 0.83),
           bbox_transform=axes.transAxes, fontsize=16)
axes.grid(True)
axes.autoscale()
# fig.savefig('Median of Previous Observations Methods Performance',
