...
rtl = axes.scatter(0, rt[0],
                   marker="o",
                   color=plt.rcParams['axes.prop_cycle'].by_key()['color'][0],
                   s=size)
axes2 = axes.twinx()
for i in range(0, len(labels)):
    axes2.scatter(i, da[i], marker="^", s=size)
dal = axes2.scatter(0, da[0],
                    marker="^",
                    color=plt.rcParams['axes.prop_cycle'].by_key()['color'][0],
                    s=size)
axes.set_xlabel('Basic Method', fontsize=16)
...
axes.set_xticks(list(range(0, len(labels))))
axes.set_xticklabels(list(range(1, len(labels)+1)))
leg1 = fig.legend(bbox_to_anchor=(1.002, 0.43),
                  bbox_transform=axes.transAxes, fontsize=12)
leg2 = fig.legend([rtl, dal],
                  ['Running Time', 'Direction Accuracy'],
                  bbox_to_anchor=(0.225, 1.01),
                  bbox_transform=axes.transAxes, fontsize=12)
axes.add_artist(leg1)
axes.grid(True)
axes.autoscale()
